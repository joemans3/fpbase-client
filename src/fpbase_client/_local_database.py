...
class FPBaseDB:
    ...
    def _create_tables(self):
        ...
        with self.conn:
            ...
            self.conn.execute('CREATE INDEX IF NOT EXISTS idx_spectra_name ON spectra(name)')
            self.conn.execute('CREATE INDEX IF NOT EXISTS idx_spectra_slug ON spectra(slug)')
    ...
    def store_spectra(self, spectra_data: List[Dict]):
        ...
        with self.conn:
            self.conn.executemany(
                ...
                [(s['name'], s['slug'], json.dumps(s['spectra'])) for s in spectra_data]
            )
